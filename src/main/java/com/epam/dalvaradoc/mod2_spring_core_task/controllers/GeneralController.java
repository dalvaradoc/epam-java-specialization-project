...
public class GeneralController {
    ...
    @ApiResponses(
            value = {
                @ApiResponse(responseCode = "200", description = "Successfullyauthenticated"),
    ...
    @Operation(
            ...
            requestBody =
                    @io.swagger.v3.oas.annotations.parameters.RequestBody(
                            description = "Current credentials and new password",
                            content =
                                    @Content(
                                            mediaType = "application/json",
                                            examples =
                                                    @ExampleObject(
                                                            value =
                                                                    SwaggerExamples
                                                                            .GENERAL_CHANGE_PASSWORD_REQBODY))))
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
                                ...
                                @Content(
                                        ...
                                        examples =
                                                @ExampleObject(
                                                        value =
                                                                SwaggerExamples
                                                                        .GENERAL_CHANGE_PASSWORD_RESBODY))),
                @ApiResponse(responseCode = "401", description = "Bad credentials")
            })
    @PutMapping("/change-password")
    public ResponseEntity<AuthenticationDTO> changePassword(
            @RequestBody Map<String, String> requestBody) {
        LOGGER.info("Trying password change for user: " + requestBody.get("username"));
        AuthenticationDTO auth =
                new AuthenticationDTO(requestBody.get("username"), requestBody.get("password"));

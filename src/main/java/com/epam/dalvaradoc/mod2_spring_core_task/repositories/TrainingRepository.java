...
public interface TrainingRepository extends CrudRepository<Training, String> {
    ...
    @Query(
            "SELECT t FROM trainings t WHERE t.trainee.username = :username AND t.date BETWEEN"
                    + " :from AND :to AND t.trainer.username = :trainerName AND t.type = :type")
    List<Training> findAllByTraineeUsername(
            String username, Date from, Date to, String trainerName, TrainingType type);

    @Query("SELECT t FROM trainings t WHERE t.trainer.username = :username")
    List<Training> findAllByTrainerUsername(String username);

    @Query(
            "SELECT t FROM trainings t WHERE t.trainer.username = :username AND t.date BETWEEN"
                    + " :from AND :to AND t.trainee.username = :traineeName")
    List<Training> findAllByTrainerUsername(
            String username, Date from, Date to, String traineeName);
}

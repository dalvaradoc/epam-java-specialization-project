...
public class TrainingService {
    ...
    public List<TrainingDTO> getTrainingsByTraineeUsername(
            ...
            Date from,
            Date to,
            @UsernameConstraint String trainerUsername,
            @NotNull TrainingType type) {
        return Optional.ofNullable(traineeUsername)
                .map(
                        username ->
                                trainingRepository.findAllByTraineeUsername(
                                        traineeUsername, from, to, trainerUsername, type))
                .map(list -> list.stream().map(mapper::toDTO).toList())
                .orElse(null);
    }
    ...
    public List<TrainingDTO> getTrainingsByTrainerUsername(
        ...
        return Optional.ofNullable(trainerUsername)
                .map(
                        username ->
                                trainingRepository.findAllByTrainerUsername(
    ...
    public TrainingDTO createTraining(@Valid TrainingDTO dto, @Valid AuthenticationDTO auth) {
        ...
        trainee =
                trainee == null
                        ? traineeRepository.findByUsername(dto.getTrainee().getAuth().getUsername())
                        : trainee;
        trainer =
                trainer == null
                        ? trainerRepository.findByUsername(dto.getTrainer().getAuth().getUsername())
                        : trainer;
    ...
    private void verifyCreatingTrainingArguments(
            TrainingDTO dto, Trainee trainee, Trainer trainer, AuthenticationDTO auth) {

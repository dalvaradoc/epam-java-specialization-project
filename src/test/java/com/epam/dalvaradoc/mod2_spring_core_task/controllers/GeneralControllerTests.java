...
class GeneralControllerTests {
  ...
  void changePasswordTest() throws Exception {
    ...
    MvcResult result = mockMvc.perform(put("/change-password")
        .contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(requestBody)))
        .andExpect(status().isOk())
        .andReturn();

    AuthenticationDTO response = objectMapper.readValue(
    ...
    mockMvc.perform(get("/login")
        .contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(new AuthenticationDTO(trainer.getUsername(), oldPassword))))
        .andExpect(status().isUnauthorized());

    // Verify new password works
    mockMvc.perform(get("/login")
        .contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(new AuthenticationDTO(trainer.getUsername(), "newTestPassword123"))))
        .andExpect(status().isOk());
  }
